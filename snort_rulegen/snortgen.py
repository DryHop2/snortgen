...
def run(args, config=None):
    try:
        config = config or {}
        offset = args.offset if args.offset is not None else get_config_value(config, "default_offset", None)
        depth = args.depth if args.depth is not None else get_config_value(config, "default_depth", None)
        if offset is not None or depth is not None:
            offset, depth = validate_offset_depth(offset, depth)

        sid = args.sid if args.sid is not None else get_config_value(config, "sid", None)
        if sid is None:
            sid = get_next_sid()
        out_path = args.outfile if args.outfile is not None else get_config_value(config, "default_outfile", "rules/local.rules")
        rev = get_latest_revision(out_path, sid)
        rule = build_rule(
            proto=args.proto if args.proto is not None else get_config_value(config, "default_proto", "tcp"),
            src_ip=args.src_ip if args.src_ip is not None else get_config_value(config, "default_src_ip", "any"),
            src_port=args.src_port if args.src_port is not None else get_config_value(config, "default_src_port", "any"),
            dst_ip=args.dst_ip if args.dst_ip is not None else get_config_value(config, "default_dst_ip", "$HOME_NET"),
            dst_port=args.dst_port if args.dst_port is not None else get_config_value(config, "default_dst_port", "80"),
            msg=args.msg if args.msg is not None else get_config_value(config, "default_msg", "Generated by SnortGen"),
            content=args.content if args.content is not None else get_config_value(config, "default_content", None),
            sid=sid,
            rev=rev,
            nocase=args.nocase if args.nocase is not None else get_config_value(config, "default_nocase", None),
            depth=depth,
            offset=offset,
            flags=args.flags if args.flags is not None else get_config_value(config, "default_flags", None),
            pcre=args.pcre if args.pcre is not None else get_config_value(config, "default_pcre", None),
            classtype=args.classtype if args.classtype is not None else get_config_value(config, "default_classtype", None),
            priority=args.priority if args.priority is not None else get_config_value(config, "default_priority", None),
            metadata=args.metadata if args.metadata is not None else get_config_value(config, "default_metadata", None),
            reference=args.reference if args.reference is not None else get_config_value(config, "default_reference", None),
        )
